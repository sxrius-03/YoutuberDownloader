...
def get_base_paths():
    ...
    Raiz/
      |-- Launcher.exe
      |-- _internal/ (dependências e pastas auxiliares no PyInstaller 6+)
          |-- bin/ (ffmpeg)
          |-- data/ (jsons)
    """
    if getattr(sys, 'frozen', False):
        # sys._MEIPASS aponta para a pasta interna com as dependências (_internal)
    ...
    return {
        "root": exe_dir,      # Pasta onde o executável reside
        "bundle": base_dir,   # Pasta onde os arquivos internos residem
...
# Chama o setup automaticamente ao importar
setup_environment()