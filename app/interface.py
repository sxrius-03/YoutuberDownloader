...
# --- CONFIGURAÇÃO UPDATE ---
CURRENT_VERSION = "2.0.1" # <-- O programa agora sabe a própria versão!
...
class BackgroundUpdateWorker(QThread):
    update_found = pyqtSignal(str, str) # nova_versao, url_do_exe
    finished_install = pyqtSignal(str) # caminho_do_novo_exe
    ...
    def check_updates(self):
        try:
            ...
            # Compara a versão do GitHub com a versão fixa do programa
            if remote_data["version"] != CURRENT_VERSION:
                self.new_version = remote_data["version"]
                self.exe_url = remote_data["exe_url"]
                self.update_found.emit(self.new_version, self.exe_url)
        except: pass
    ...
    def install_updates(self):
        try:
            ...
            if r.status_code == 200:
                ...
                pasta_base = os.path.dirname(exe_atual)
                novo_exe = os.path.join(pasta_base, "update_new.exe")

                with open(novo_exe, 'wb') as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        f.write(chunk)
                
                self.finished_install.emit(novo_exe)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        # Lê a versão automaticamente no título!
        self.setWindowTitle(f"YouTube Downloader - v{CURRENT_VERSION}")
    ...
    def ask_for_update(self, new_ver, exe_url):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self.lbl_status.setText("Baixando nova versão (isso pode levar um tempo)...")
            self.updater.exe_url = exe_url
            self.updater.mode = "install"
            self.updater.start() 

    def on_update_installed(self, novo_exe_path):
        QMessageBox.information(self, "Atualizado", "Download concluído!\nO programa será reiniciado agora para aplicar a nova versão.")
        
        exe_atual = sys.executable if getattr(sys, 'frozen', False) else os.path.abspath(sys.argv[0])
        pasta_base = os.path.dirname(exe_atual)
        bat_path = os.path.join(pasta_base, "atualizador.bat")
        
        nome_exe_atual = os.path.basename(exe_atual)
        nome_novo_exe = os.path.basename(novo_exe_path)
        
        bat_content = f"""@echo off
timeout /t 2 /nobreak > NUL
del "{nome_exe_atual}"
ren "{nome_novo_exe}" "{nome_exe_atual}"
